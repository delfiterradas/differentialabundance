...
workflow DIFFERENTIALABUNDANCE {
    ...
    ch_proteus_raw = prepareModuleOutput(PROTEUS.out.raw_tab, ch_paramsets, ['contrast'], true)
        .groupTuple()
        .map { meta, files -> [ meta, files[0] ] }
    ch_proteus_norm = prepareModuleOutput(PROTEUS.out.norm_tab, ch_paramsets, ['contrast'], true)
        .groupTuple()
        .map { meta, files -> [ meta, files[0] ] }
    ...
    ch_differential_norm = prepareModuleOutput(ABUNDANCE_DIFFERENTIAL_FILTER.out.normalised_matrix, ch_paramsets, ['differential_method'], true) // meta, norm file
    ch_differential_varstab = prepareModuleOutput(ABUNDANCE_DIFFERENTIAL_FILTER.out.variance_stabilised_matrix, ch_paramsets, ['differential_method'], true) // meta, varstab file
